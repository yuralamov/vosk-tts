...
def main() -> None:
    p = argparse.ArgumentParser(description="Synthesize speech with Higgs Audio v3 TTS (native).")
    p.add_argument("text", help="Text to synthesize (supports inline control tokens).")
    p.add_argument("-o", "--output", default="output.wav", help="Output WAV path.")
    p.add_argument("--model-dir", default="higgs-audio-v3-tts-4b", help="Where weights are stored/downloaded.")
    p.add_argument("--ref-audio", help="Reference audio file for zero-shot voice cloning.")
    p.add_argument("--ref-text", default="", help="Transcript of the reference audio.")
    ...
    p.add_argument("--max-new-tokens", type=int, default=1024, help="~1024 ≈ 25-30 s of audio.")
    ...
    model_dir = download_model(Path(args.model_dir).expanduser())
    bundle = load_bundle(model_dir, args.device, args.dtype, args.attention)

    logger.info("Synthesizing...")
    audio = synthesize(
        bundle, args.text,
        reference_audio=args.ref_audio, reference_text=args.ref_text,
        max_new_tokens=args.max_new_tokens, temperature=args.temperature,
        top_p=args.top_p, top_k=args.top_k, seed=args.seed,
    )
    torchaudio.save(args.output, audio.view(1, -1), SAMPLE_RATE)
    logger.info("Wrote %s (%.1f s, %d Hz)", args.output, audio.numel() / SAMPLE_RATE, SAMPLE_RATE)
